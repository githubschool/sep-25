...
Every repository contains the ability to [administer](https://help.github.com/categories/administering-a-repository/) it, for visibility, collaboration and quality checking as an example. Check out each of the settings to learn how you can find tune your environment.
...
1. Enter a **Title** indicating you will create a file with your name on it. Example:
    ```
...
1. In the repository's **Code** tab, navigate to the `workshop-attendees` folder.
1. While in the `workshop-attendees` folder, click **Create new file**.
1. On the **Create new file** screen, be sure to provide a new filename.
1. In the **Edit new file** box, type some info about yourself, for example:
    ```
...
1. In the top-right-hand corner, click **Fork**. If asked, fork to your personal GitHub account.
...
1. Check out the new Webhook created in your GitHub repository. Go to the repository's **Settings** tab, then select **Webhooks** to see that it was automatically created when you authorized it.
...
1. In the Heroku **Deploy** tab, for the **Deployment method** select :octocat: **GitHub** and **Connect to GitHub**. Log in if prompted, but it should take your already logged-in session, where you can click **Authorize heroku**.
